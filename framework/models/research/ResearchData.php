...
class ResearchData {
	...
	public function __construct( $_id, Array $aqr= null ){
        if( $aqr == null ) {
            global $DB;
    ...
    public static function GetAll(){
        global $DB;
        $ret= Array();
        $qr= $DB->query("select * from ".TB_PREFIX."t_research");
        while($row= $qr->fetch_array()) $ret[]= new ResearchData(0, $row);
        return $ret;
    }
    ...
    public function GetResearchTime( $_nextLevel ){
        ...
        $time = ( $this->researchTime * $this->timeMultiplier * $_nextLevel ) / Config::Instance()->researchfast_molt;
        return (int)$time;
    }
};
...
class Research {
    ...
    public function __construct( ResearchData $researchData, $level = 0, $researchEndTime = 0 ){
