ALTER TABLE `%PREFIX%ally_charge` ADD `adm_forum` tinyint(1) NOT NULL;
ALTER TABLE `%PREFIX%ally_charge` ADD `adm_full` tinyint(1) NOT NULL;
ALTER TABLE `%PREFIX%ally_charge` ADD `adm_recruit` tinyint(1) NOT NULL;
ALTER TABLE `%PREFIX%ally_charge` ADD `adm_reliable` tinyint(1) NOT NULL;
ALTER TABLE `%PREFIX%ally_charge` ADD `adm_wars` tinyint(1) NOT NULL;
ALTER TABLE `%PREFIX%ally_charge` DROP `admin`;
...
ALTER TABLE `%PREFIX%ally_pact` ADD `data_end` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP;
ALTER TABLE `%PREFIX%ally_pact` ADD `data_start` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP;
...
UPDATE `%PREFIX%conf` SET `sge_ver`= 145 WHERE `sge_ver`= 144;